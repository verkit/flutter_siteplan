...
class KavlingWidget extends StatefulWidget {
  KavlingWidget({this.scale = 1, this.value, this.groupKavling, this.pointkus, this.onChanged, this.onTapDown, this.onTapUp, this.onTap, this.color0, this.color1, this.color2, this.x, this.y, this.width, this.height, this.shapes});
  final double scale;
  final int value;
...
class _KavlingWidgetState extends State<KavlingWidget> {
  ...
  ShapeBorder Kavling(BuildContext context) {
    if (widget.shapes == 'rect') {
      return KavlingShape(scales: widget.scale, x: widget.x, y: widget.y, width: widget.width, height: widget.height, shapes: widget.shapes, color: _color);
    } else if (widget.shapes == 'polygon') {
      return KavlingShape(scales: widget.scale, x: widget.x, y: widget.y,pointkus: widget.pointkus, shapes: 'polygon', width: widget.width, height: widget.height, color: _color);
    }
  }
}
